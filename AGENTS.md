...
- Field mappings are data, not code: edit `Salesforce Field Mapping` records in the site. `setup/default_mappings.py` seeds them on install and `ensure_default_field_mappings` self-heals missing rows on every migrate and sync tick (additive: never overwrites user customisations). One narrow exception: rows in `_TRANSFORM_CONVERGENCES` whose transform is inert (empty/`"none"` — the field's default, so UI-added rows regenerate it) are converged to the required transform, because inert is known-broken there. Adding a SOQL field requires a mapping row, not a syncer change.
